...
class jobdescription extends BasePage_1.BasePage {
    ...
    async writeJDContent() {
        try {
            ...
            if (currentUrl.includes('/null/')) {
                console.log(`⚠️ ${Selectors_1.WARNING_MESSAGES.NULL_JOB_ID}`);
                console.log('⚠️ Consider ensuring job is properly created before writing JD');
            }
            // Extract job ID from URL if possible
            const jobIdMatch = currentUrl.match(Selectors_1.REGEX_PATTERNS.JOB_ID_EXTRACTION);
            ...
            await this.commonUtils.writeToTextEditor(jdContent, [...Selectors_1.JOB_DESCRIPTION_PAGE.TEXT_EDITOR_SELECTORS]);
        }
        catch (error) {
    ...
    async scrollToBottom() {
        await this.commonUtils.scrollToBottom(CommonUtils_1.CommonUtils.TIMEOUTS.SCROLL_ATTEMPTS_SHORT);
    }
    /**
     * Click the Job Score Criteria button
     */
    async clickJobScoreCriteria() {
        const button = await this.commonUtils.findElementWithFallback([...Selectors_1.JOB_DESCRIPTION_PAGE.JOB_SCORE_CRITERIA_BUTTON_SELECTORS]);
        if (!button) {
            throw new Error(Selectors_1.ERROR_MESSAGES.JOB_SCORE_CRITERIA_NOT_FOUND);
        }
        // Scroll the button into view before clicking
        await button.scrollIntoViewIfNeeded();
        await this.commonUtils.waitForTimeout(CommonUtils_1.CommonUtils.TIMEOUTS.AFTER_SCROLL);
        // Click the button
    ...
    async saveJobDescription() {
        try {
            ...
            console.log('✓ Current URL:', currentUrl);
            if (currentUrl.includes('/null/')) {
                console.log(`⚠️ ${Selectors_1.WARNING_MESSAGES.NULL_JOB_ID_GRACEFUL}`);
                // Try to wait for the page to load and get a proper job ID
                await this.commonUtils.waitForTimeout(CommonUtils_1.CommonUtils.TIMEOUTS.AFTER_UUID_RETRY);
                ...
                if (newUrl.includes('/null/')) {
                    console.log(`⚠️ ${Selectors_1.WARNING_MESSAGES.STILL_NULL_JOB_ID}`);
                }
            }
            // Look for save button
            const saveButtonFound = await this.commonUtils.clickButtonWithFallback([...Selectors_1.JOB_DESCRIPTION_PAGE.SAVE_BUTTON_SELECTORS], 'save button');
            if (!saveButtonFound) {
                console.log(`⚠️ ${Selectors_1.WARNING_MESSAGES.NO_SAVE_BUTTON}`);
            }
            // Check for any error messages on the page
            await this.commonUtils.checkForErrorMessages();
            console.log(`✅ ${Selectors_1.SUCCESS_MESSAGES.JD_SAVE_COMPLETED}`);
        }
        catch (error) {
            ...
            if (error.message && error.message.includes('UUID')) {
                console.log(`⚠️ ${Selectors_1.ERROR_MESSAGES.UUID_ERROR_DETECTED}`);
                Selectors_1.ERROR_MESSAGES.UUID_ERROR_CAUSES.forEach((cause, index) => {
